...
<div class="container my-3">
    <!-- 질문 -->
    <h2 class="border-bottom py-2"><strong>{question.subject}</strong></h2>
    <div class="card my-3">
        <div class="card-body">
    ...
    <form method="post" class="my-3">
        ...
        class="btn btn-primary { $is_login ? 'btn-primary' : 'btn-dark' } { $is_login ? '' : 'disabled' }"
        on:click="{post_answer}" />
        <button class="btn btn-primary" type="button" on:click="{()=>{push('/')}}">목록으로</button>
        {#if $is_login}<button class="btn btn-primary" type="reset">초기화</button>{/if}
    </form>
</div>